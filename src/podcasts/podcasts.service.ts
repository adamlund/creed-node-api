...
export class PodcastsService {
  ...
  async fetchGenre(genreId: number): Promise<GenreDto> {
    ...
    if (!genre) {
      throw new BadRequestException(
        `Unsupported genre_id provided, genre_id=${genreId}`,
      );
    }
    return genre;
  }
  ...
  ): Promise<Partial<PodcastResultDto>> {
    const genre = query.genre_id
      ? await this.fetchGenre(query.genre_id)
      : this.defaultGenre();

    // sanitize page inputs
